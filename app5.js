...
var expressSession = require("express-session"); //  세션 미들웨어
var mongoose = require("mongoose"); // 몽구스 모듈 불러들이기
var crypto = require("crypto"); // crypto 모듈 불러들이기
...
function connectDB() {
  ...
  function createUserSchema() {
    ...
    UserSchema = mongoose.Schema({
      ...
      name: { type: String, index: "hashd", default: " " },
      age: { type: Number, default: -1 },
      created_at: { type: Date, index: { unique: false }, default: Date.now },
      updated_at: { type: Date, index: { unique: false }, default: Date.now },
    });

    // UserSchema 버츄얼 속성 - 몽고디비에 저장되지 않음.
